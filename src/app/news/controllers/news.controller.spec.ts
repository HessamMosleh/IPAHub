...
describe('NewsController', () => {
  ...
  it('passes the public query dto through to NewsService.findAll and returns its paginated result', async () => {
    // Arrange
  ...
  it('forwards the id param to NewsService.findById', async () => {
    const post = buildNews();
    mockService.findById.mockResolvedValue(post);

    const result = await controller.findById(FIXED_NEWS_ID);

    expect(mockService.findById).toHaveBeenCalledTimes(1);
    expect(mockService.findById).toHaveBeenCalledWith(FIXED_NEWS_ID);
  ...
  it('propagates service errors to the caller (no swallowing)', async () => {
    ...
    await expect(controller.findById(FIXED_NEWS_ID)).rejects.toBe(err);
  });
});
...
describe('news-test-fixtures', () => {
  it('exports a fixed, deterministic news id', () => {
    expect(FIXED_NEWS_ID).toBe('507f1f77bcf86cd799439011');
