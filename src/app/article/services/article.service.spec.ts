...
describe('ArticleService', () => {
  ...
  let mockModel: ReturnType<typeof buildArticleModelMock>;
  ...
  beforeEach(async () => {
    mockModel = buildArticleModelMock();
  ...
  describe('findAll', () => {
    it('returns published article with default pagination and an active-status guard', async () => {
      ...
      expect(mockModel.find).toHaveBeenCalledWith(
        expect.objectContaining({ status: ArticleStatus.ACTIVE }),
      );
    ...
    it('filters by category and province', async () => {
      ...
      expect(filter.status).toBe(ArticleStatus.ACTIVE);
    ...
    it('omits $or when no search term is provided', async () => {
      ...
      await service.findAll({ category: ArticleCategory.NATIONAL });
  ...
  describe('findById', () => {
    it('returns an active article by valid MongoDB ObjectId', async () => {
      const item = buildArticle();
      mockModel.findOne.mockReturnValue(buildQueryChain(item));

      const result = await service.findById(FIXED_ARTICLE_ID);
    ...
    it('throws NotFoundException when no row matches', async () => {
      ...
      await expect(service.findById(FIXED_ARTICLE_ID)).rejects.toThrow(
        NotFoundException,
      );
    });
